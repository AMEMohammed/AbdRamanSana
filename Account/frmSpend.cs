...
namespace Account
{
    public partial class frmSpend : Form
    {
        ...
        public frmSpend(string ServNm, string DbNm, string UesrSql, string PassSql, int UserId)
        {
            InitializeComponent();
            try
            {
        ...
        void LoadDate()
        {
           ...
           // جلب اسم حسابات الصرف والقبض
            ...
            {
                dataGridView1.DataSource = Acn.GetSpendByDate(dateTimePicker1.Value, dateTimePicker1.Value.AddDays(1));
            }
            txtMount.Text = "";
            txtNote.Text = "";
            textBoxmnalt.Text = "";
            ...
            {     // العملة
                ...
                {   //الحساب
                    ...
                    {  // مناولة
                        ...
                        {
                            MessageBox.Show("يجب ملاء حقل المناولة");
                        }
                    }
                    else
                    {
        ...
        private void dataGridView1_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    comboBoxAccountClient.Text = dataGridView1.SelectedRows[0].Cells["حساب الصرف"].Value.ToString();
                    ...
                    textBoxmnalt.Text = dataGridView1.SelectedRows[0].Cells["مناولة"].Value.ToString();
                    txtNote.Text = dataGridView1.SelectedRows[0].Cells["ملاحظات"].Value.ToString();
                    ID = Convert.ToInt32(Acn.GetIDSpendByNumber(Convert.ToInt32(dataGridView1.SelectedRows[0].Cells["الرقم"].Value.ToString())));
                            ...
                            if (Convert.ToInt32(comboBoxAccoutnCashier.SelectedValue) > 0)
                            {
                                if (ID > 0 && TheNumber > 0)
                                {
                                    if (MessageBox.Show("هل تريد الاستمرار بعملية التعديل", "تنبيه", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                                    {
                                        ...
                                        Acn.UpdateSpend(ID, TheNumber, dateTimePicker1.Value.Date, Convert.ToDouble(txtMount.Text), Convert.ToInt32(comboBoxCurrncy.SelectedValue), Convert.ToInt32(comboBoxAccoutnCashier.SelectedValue), Convert.ToInt32(comboBoxAccountClient.SelectedValue), txtNote.Text, textBoxmnalt.Text, IDUSER, comboBoxAccountClient.Text, comboBoxAccoutnCashier.Text);

                                        dataGridView1.DataSource = Acn.GetSpendByNumber(TheNumber);
                                        ID = 0;
                                        TheNumber = 0;
                                    }
                                }
        ...
        private void txtMount_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.Shift && e.KeyCode == Keys.Enter)
                {
                    ...
                    if (txtMount.Text.Length > 0)
                    {
                        dataGridView1.DataSource = Acn.GetSpendByAmount(txtMount.Text, dateTimePicker1.Value.Date, dateTimePicker1.Value.Date.AddDays(1));
        ...
        private void comboBoxCurrncy_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.Shift && e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        dataGridView1.DataSource = Acn.GetSpendByCurnncyNm(comboBoxCurrncy.Text, dateTimePicker1.Value.Date, dateTimePicker1.Value.Date.AddDays(1));
        ...
        private void comboBoxAccountClient_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.Shift && e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        dataGridView1.DataSource = Acn.GetSpendByAccountNm(comboBoxAccountClient.Text, dateTimePicker1.Value.Date, dateTimePicker1.Value.Date.AddDays(1));
        ...
        private void تصدرالكلالىاكسلToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                for (int i = 0; i < dataGridView1.SelectedRows[0].Cells.Count; i++)
                {
                    ...
                    dataGridView1.SelectedRows[0].Cells["العملة"].Value.ToString(),dataGridView1.SelectedRows[0].Cells["حساب الصرف"].Value.ToString()
                    ,dataGridView1.SelectedRows[0].Cells["الصندوق"].Value.ToString(),dataGridView1.SelectedRows[0].Cells["التاريخ"].Value.ToString()
                    ,dataGridView1.SelectedRows[0].Cells["مناولة"].Value.ToString(),dataGridView1.SelectedRows[0].Cells["ملاحظات"].Value.ToString()
                   
                    ,Acn.GetUserNmByID(IDUSER)});
                }
                FrmRports.frmReprt f = new FrmRports.frmReprt(dt, 4);
                f.ShowDialog();
            }
        }
        //btn Searching
        private void btnSearcing_Click(object sender, EventArgs e)
        {
            ...
            if (falg == false)
            {
                ...
                Dc.Add("حساب الصرف", "AccountClient");
                Dc.Add("الصندوق", "Accountcashier");
                Dc.Add("مناولة", "Delivery");
                Dc.Add("ملاحظات", "Note");
                ...
                Transfer.frmSearch f = new Transfer.frmSearch(Sern, DbNm, userSql, passSql, 1, Dc, "سند صرف", "بحث في صرف من حساب");
                f.GetData = RetunData;
                f.Name = "Search";
